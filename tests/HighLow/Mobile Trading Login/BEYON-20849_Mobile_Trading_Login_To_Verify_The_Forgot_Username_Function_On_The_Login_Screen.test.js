...
test('BEYON-5016 - Mobile Trading Login - Forgot Username - To Verify The Forgot Username Function On The Login Screen', async ({ page, context }) => {
    ...
    const mobileTradingLoginForgotUsernameLink = mobile.locator('//a[contains(text(),"ユーザー名、")]');
    await mobileTradingLoginForgotUsernameLink.click();

    //Enter Invalid Details
    const mobileTradingLoginForgotUsernameDOBField = mobile.locator('//*[@id="forgot_username_dob"]');
    await expect(mobileTradingLoginForgotUsernameDOBField).toBeVisible();
    await mobileTradingLoginForgotUsernameDOBField.click();
    await mobileTradingLoginForgotUsernameDOBField.fill(crmTraderDOB);
    const mobileTradingLoginForgotUsernameEmailField = mobile.locator('//*[@id="forgot_username_email"]');
    await expect(mobileTradingLoginForgotUsernameEmailField).toBeVisible();
    await mobileTradingLoginForgotUsernameEmailField.click();
    await mobileTradingLoginForgotUsernameEmailField.fill("invalidemailaddress@gmail.com");
    const mobileTradingLoginForgotUsernameSubmitButton = mobile.locator('//*[@id="forgot_username_submit"]');
    await mobileTradingLoginForgotUsernameSubmitButton.click();

    //Check Displayed Error Message
    const invalidEmailErrorMessage = mobile.locator('//div[text()="口座が見つかりませんでした。"]');
    await expect(invalidEmailErrorMessage).toBeVisible();
    ...
    await mobileTradingLoginForgotUsernameDOBField.fill(crmTraderDOB);
    await mobileTradingLoginForgotUsernameEmailField.click();
    await mobileTradingLoginForgotUsernameEmailField.fill(traderEmail);

    //Submit Form
    await mobileTradingLoginForgotUsernameSubmitButton.click();

    //Confirm Success Screen Is Displayed
    const mobileTradingLoginForgotUsernameSuccessMessage = mobile.locator('//div[contains(@class, "standaloneForm_subheadline__") and contains(text(), "ユーザー名を送信しましたのでメールボックスをご確認ください。迷惑フォルダもご確認ください。")]');
    await expect(mobileTradingLoginForgotUsernameSuccessMessage).toBeVisible();

    //Click Email Log Tab
    await crmTab.bringToFront();
    const crmSnapshotEmailLogTab = crmTab.locator('//descendant::ul[contains(@class, "nav-pills")][1]/li[5]/a');
